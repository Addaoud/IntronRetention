...
def get_indices(dataset_size: int, test_split: float, output_dir: str, shuffle_data=True, seed_val=100):
    ...
    and os.path.exists(test_indices_path):
        ...
        train_indices, valid_indices, test_indices = indices[2*split_val:], indices[:split_val], indices[split_val:2*split_val]
...
class datasetLR(Dataset):
    ...
    def __getitem__(self,index):
        return self.data[index],self.target[index].float().reshape(-1,)
    def get_labels(self):
        return self.target
    def __len__(self):
        return self.len
...
class DatasetLoad(Dataset):
    def __init__(self, df_path, fa_file, lazyLoad=False):
        ...
        self.df_final = pd.merge(self.df_seq_all['header',"sequence"],self.df_test['header','label'],on='header',how='inner')
        ...
        if not self.lazyLoad:
            ...
            for i in range(0,self.df_final.shape[0]):
                ...
                self.One_hot_Encoded_Tensors.append(torch.tensor(hot_encode_sequence(X)))
